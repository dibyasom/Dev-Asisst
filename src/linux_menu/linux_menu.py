...
psy.speak("Please Choose the options provided below")
...
while True:
    ...
    if ("create a folder" in option) or ("folder" in option) :
    ...
    elif option == "install pkg" or option == "install package" :
    ...
    elif option == "ping address" or option == "ping ip address" :
        ip = input("Enter the ip address (default name '127.0.0.1') : ") or "127.0.0.1"
        ping(ip)
    ...
    elif option == "show all using port in system" or option == "show open ports" :
        netstat = netstat()
        print(netstat)
